...
const parameterRows = [
  ['Peat fire vulnerability (PFVI)', '0.82', 'peatfr demo ensemble', 'Modelled', 'https://github.com/mellygsln/peatfr'],
  ['Active-fire evidence', 'VIIRS high, 3 h old', 'NASA FIRMS', 'Representative', 'https://firms.modaps.eosdis.nasa.gov/'],
  ['Rainfall, prior 24 h', '1.8 mm', 'Weather fallback', 'Representative', 'https://gpm.nasa.gov/data/imerg'],
  ['Air temperature', '32.1 C', 'Weather fallback', 'Representative', 'https://cds.climate.copernicus.eu/datasets/reanalysis-era5-land'],
  ['Wind', '13 km/h from SE', 'Weather fallback', 'Representative', 'https://cds.climate.copernicus.eu/datasets/reanalysis-era5-land'],
  ['Soil moisture', '0.18 m3/m3', 'ERA5-Land proxy', 'Representative', 'https://cds.climate.copernicus.eu/datasets/reanalysis-era5-land'],
  ['Water-table depth', 'Unavailable', 'No local sensor', 'Missing', ''],
  ['PM2.5', '115.6 ug/m3', 'BMKG station context', 'Representative', 'https://www.bmkg.go.id/kualitas-udara/pm25'],
  ['Nearest housing', '7.0 km', 'OpenStreetMap context', 'Representative', 'https://www.openstreetmap.org/'],
  ['Estimated exposed population', '2,340', 'Prototype exposure model', 'Representative', ''],
  ['Nearest road / waterway', '3.8 km / 1.2 km', 'OpenStreetMap context', 'Representative', 'https://www.openstreetmap.org/'],
];
...
export default function Home() {
  ...
  const [context, setContext] = useState<{ source: string; observedAt: string; weather: Record<string, number>; air: Record<string, number> } | null>(null);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <section className="map-panel" aria-label="Central Kalimantan risk map"><div className="map-toolbar"><strong>Central Kalimantan / Kalimantan context</strong><span>Zoom and pan enabled</span></div><OperationsMap incidents={activeIncidents} selectedId={selected.id} onSelect={setSelectedId} layers={layers} /><div className="layer-bar">{(['hotspot', 'peat', 'wind'] as const).map((key) => <button key={key} className={layers[key] ? 'layer-on' : ''} onClick={() => setLayers((current) => ({ ...current, [key]: !current[key] }))}>{layers[key] ? 'Hide' : 'Show'} {key}</button>)}<span className="layer-pending">Risk and exposure layers: pending source data</span></div></section>
      <aside className="detail-panel"><div className="panel-heading"><div><p className="eyebrow">Selected incident</p><h2>{selected.place}</h2><span>{selected.region}</span></div><span className={`priority-badge ${selected.priority}`}>{priorityLabel[selected.priority]}</span></div><div className="score-row"><div><span>Risk</span><strong>{selected.risk}%</strong><small>next {horizon}</small></div><div><span>Confidence</span><strong>{selected.confidence}%</strong><small className={selected.confidenceBand}>{selected.confidenceBand}</small></div></div><div className="action-box"><p>Recommended action</p><strong>{selected.action}</strong><span>{selected.deadline}</span></div><section className="evidence"><h3>Why this is ranked</h3>{selected.evidence.map((item) => <div key={item}><button className="evidence-button" onClick={() => setExpandedReason(expandedReason === item ? null : item)}><i />{item}<b>{expandedReason === item ? 'Hide' : 'Inspect'}</b></button>{expandedReason === item && <div className="evidence-detail">{evidenceExplanation(item)}</div>}</div>)}</section><details className="parameter-panel"><summary>All inputs, sources, and data quality</summary><div className="parameter-table">{context ? <><div><span>Live temperature</span><b>{context.weather.temperature_2m} C</b><small>Open-Meteo · {context.observedAt} UTC</small></div><div><span>Live wind</span><b>{context.weather.wind_speed_10m} km/h</b><small>Open-Meteo · direction {context.weather.wind_direction_10m} degrees</small></div><div><span>Live precipitation</span><b>{context.weather.precipitation} mm</b><small>Open-Meteo · current observation</small></div><div><span>Live PM2.5</span><b>{context.air.pm2_5} ug/m3</b><small>Open-Meteo Air Quality · current estimate</small></div></> : <div><span>Live weather and PM2.5</span><b>Loading</b><small>Open-Meteo coordinate lookup</small></div>}{parameterRows.map(([name, value, source, quality, url]) => <div key={name}><span>{name}</span><b>{value}</b><small>{url ? <a href={url} target="_blank" rel="noreferrer">{source}</a> : source} · {quality}</small></div>)}</div></details><div className="limit-box"><strong>Confidence limitation</strong><span>{selected.limitation}</span></div><div className="air-readout"><span>Air quality context</span><b>{context ? `${context.air.pm2_5} ug/m3 live estimate` : selected.pm25}</b></div><div className="field-actions"><p>Field feedback</p><div><button onClick={() => updateStatus('Verified fire')}>Verified fire</button><button onClick={() => updateStatus('False alarm')}>False alarm</button><button onClick={() => updateStatus('Monitoring')}>Monitor</button></div></div></aside>
